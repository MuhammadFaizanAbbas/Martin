const leadsPage = (function () {
    ...
    <div id="editPanel" class="side-panel">
      ...
      <div class="side-panel-body">
        <form id="editForm">
          ...
          <div class="form-group"><label>Baujahr Dach</label><select id="editBaujahr"><option value="">Wählen...</option>${Array.from({length:80},(_,i)=>2024-i).map(y=>`<option value="${y}">${y}</option>`).join("")}</select></div>
  ...
  const addLeadsStyles = () => {
    ...
    styles.textContent = `
      ...
      /* ── Modals ── */
      .modal-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 3000; justify-content: center; align-items: center; }
  ...
  function getStatusClass(status) {
    ...
    if (status === "Infos eingeholt" || status === "Nur Info eingeholt") return "badge-info";
    if (status === "Beauftragung" || status === "EA Beauftragung" || status === "NF Beauftragung") return "badge-beauft";
    if (status === "In Bearbeitung" || status === "in Bearbeitung") return "badge-bearbeitung";
    return "badge-offen";
  }
  ...
  function mapAPIToLead(apiLead) {
    ...
    return {
      ...
      summe: apiLead.summe_netto && apiLead.summe_netto !== "0.00"
        ? `€ ${formatNumber(apiLead.summe_netto)}`
        : "€ 0,00",
      datum: apiLead.created_at
        ? apiLead.created_at.split(" ")[0]
        : (apiLead.datum && apiLead.datum !== "0000-00-00" ? apiLead.datum : "—"),
      nachfassen: apiLead.nachfassen !== "0000-00-00" && apiLead.nachfassen ? apiLead.nachfassen : "",
      salutation: apiLead.salutation || "",
      briefberatungTelefon: apiLead.erstberatung_telefon === "WAHR" ? "Ja" : (apiLead.erstberatung_telefon === "FALSCH" ? "Nein" : ""),
  ...
  window.editLead = id => {
    ...
    document.getElementById("editBriefberatungTelefon").value = lead.briefberatungTelefon || "";
    document.getElementById("editStrasseObjekt").value = lead.strasseObjekt || "";
    ...
    document.getElementById("editDacheindeckung").value = lead.dacheindeckung || "";
  ...
  window.viewLead = id => {
    ...
    document.getElementById("viewTabNotes").innerHTML = lead.notes?.length
      ? lead.notes.map(n=>`<div class="note-card"><div class="note-text">${escapeHtml(n.text)}</div><div class="note-meta"><span>${escapeHtml(n.author)}</span><span>${escapeHtml(n.date)}</span></div></div>`).join("")
    ...
    document.querySelectorAll(".view-tab").forEach(tab => {
      tab.onclick = () => {
        ...
        document.getElementById(`viewTab${key.charAt(0).toUpperCase()+key.slice(1)}`).classList.add("active");
      };
    });
  ...
  function renderNotesList() {
    ...
    list.innerHTML = lead.notes.map(n=>`<div class="note-card"><div class="note-text">${escapeHtml(n.text)}</div><div class="note-meta"><span>${escapeHtml(n.author)}</span><span>${escapeHtml(n.date)}</span></div></div>`).join("");
  }
  ...
  function saveNote() {
    ...
    lead.notes.push({ text: txt, author: "Martin Schwaak", date: `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())} ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}` });
    document.getElementById("noteInput").value = "";
    renderNotesList();
  }
  ...
  function collectForm() {
    return {
      ...
      briefberatungTelefon: document.getElementById("editBriefberatungTelefon").value,
  ...
  async function init(contentEl, titleElement) {
    ...
    // Panel close
    document.getElementById("closeEditPanel")?.addEventListener("click", closePanel);
    ...
    // View modal
    document.getElementById("closeViewModal")?.addEventListener("click", () => document.getElementById("viewModal").classList.remove("active"));
    document.getElementById("viewModal")?.addEventListener("click", e => {
      if (e.target === document.getElementById("viewModal")) document.getElementById("viewModal").classList.remove("active");
    });

    // Notes modal
    document.getElementById("closeNotesModal")?.addEventListener("click", () => document.getElementById("notesModal").classList.remove("active"));
    document.getElementById("notesModal")?.addEventListener("click", e => {
      if (e.target === document.getElementById("notesModal")) document.getElementById("notesModal").classList.remove("active");
    });
